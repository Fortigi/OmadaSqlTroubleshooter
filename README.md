...
- A log file per session, written continuously and flushed as it goes, so a crashed session still
  has a log — in `%APPDATA%\OmadaSqlTroubleshooter\logs`, shown in the log window, redacted exactly
  as the window is, and pruned automatically
...
| `%APPDATA%\OmadaSqlTroubleshooter\logs\OmadaSqlTroubleshooter_<start>_pid<id>_<part>.log` | One log file per session, written continuously for the lifetime of the application and flushed line by line so it survives a crash. It contains exactly what the log window contains — the same text, through the same redaction layer, so credentials, tokens, session cookies and result values are masked before anything is written. Query text is masked too, unless you tick **Show request body** or start with `-SkipBodyRedaction`, in which case the query reaches this file just as it reaches the window. | NTFS permissions on your profile | Pruned on start-up: 14 days and 20 sessions by default, and each file is capped at 20 MB | Delete the folder, or set `EnableSessionLogFile` to `false` in the settings file to stop writing it |
