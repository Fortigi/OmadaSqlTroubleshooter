...
- Optional session log file (off by default; set `EnableSessionLogFile` to `true`), written
  continuously and flushed as it goes, so a crashed session still has a log — in
  `%APPDATA%\OmadaSqlTroubleshooter\logs`, shown in the log window, redacted exactly as the window
  is, split into 5 MB parts, and pruned to the 10 most recent sessions
...
| `%APPDATA%\OmadaSqlTroubleshooter\logs\OmadaSqlTroubleshooter.log` and `OmadaSqlTroubleshooter_<start>_<part>.log` | **Only when you set `EnableSessionLogFile` to `true` in the settings file; off by default.** A session log, written continuously for the lifetime of the application and flushed line by line so it survives a crash. The running session writes `OmadaSqlTroubleshooter.log`; past 5 MB, and at the next start, it is renamed to a numbered part named for the session's start time. It contains exactly what the log window contains — the same text, through the same redaction layer, so credentials, tokens, session cookies and result values are masked before anything is written. Query text is masked too, unless you tick **Show request body** or start with `-SkipBodyRedaction`, in which case the query reaches this file just as it reaches the window. | NTFS permissions on your profile | Pruned on start-up to the 10 most recent sessions, all parts of a session counting as one; no size or age limit | Delete the folder, or set `EnableSessionLogFile` back to `false` to stop writing it |
