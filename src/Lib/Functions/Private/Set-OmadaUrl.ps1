function Set-OmadaUrl {
    ...
    try {
        ...
        if (![string]::IsNullOrWhiteSpace($Script:MainFormForm.Elements.TextBoxURL.Text)) {
            ...
            $Uri = $null
            try {
                $Uri = [System.Uri]::new($Script:MainFormForm.Elements.TextBoxURL.Text.Trim())
            }
            catch {}

            if ($null -eq $Uri) {
                if ($Script:MainFormForm.Elements.TextBoxURL.Text -notlike "http*") {
                    if ($Script:MainFormForm.Elements.TextBoxURL.Text -notlike "*.*" -and $Script:MainFormForm.Elements.TextBoxURL.Text -notlike "*.omada.cloud") {
                        $Script:MainFormForm.Elements.TextBoxURL | Set-TextBlockText -Text "https://$($Script:MainFormForm.Elements.TextBoxURL.Text).omada.cloud"
                    }
                    else {
                        $Script:MainFormForm.Elements.TextBoxURL | Set-TextBlockText -Text "https://$($Script:MainFormForm.Elements.TextBoxURL.Text)"
                    }
                }
            }
            elseif ($Uri.Scheme -ne 'http' -and $Uri.Scheme -ne 'https') {
                "Input Url {0} is not valid." -f $Script:MainFormForm.Elements.TextBoxURL.Text.Trim() | Write-LogOutput -LogType ERROR
            }
        ...
        $Script:MainFormForm.Elements.TextBoxURL | Set-TextBlockText -Text $null
