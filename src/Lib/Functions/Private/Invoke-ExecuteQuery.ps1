            ...
            try {
                if ($Script:Task.Status -eq "RanToCompletion") {
                    ...
                    if ($null -ne $Script:RunTimeData.QueryResult -and ($Script:RunTimeData.QueryResult.d.Rows | Measure-Object).Count -le 0) {
                        ...
                        $Script:MainFormForm.Elements.DataGridQueryResult.ItemsSource = $null
                    }
                    else {
                        $Script:MainFormForm.Elements.DataGridQueryResult.AutoGenerateColumns = $true
                        try {
                            $Script:MainFormForm.Elements.DataGridQueryResult.ItemsSource = @($Script:RunTimeData.QueryResult.d.Rows)
                        }
                        catch {
                            #Work-around issue that Omada can return invalid JSON keys.
                            $Script:MainFormForm.Elements.DataGridQueryResult.ItemsSource = @(($Script:RunTimeData.QueryResult | ConvertTo-Json -Depth 10 | Invoke-SanitizeJsonKeys | ConvertFrom-Json -Depth 10).d.Rows)
                        }
                        ...
                        $Script:MainFormForm.Elements.ButtonShowOutput.IsEnabled = $true
                        $Script:MainFormForm.Elements.ButtonSaveOutputFile.IsEnabled = $true
                        "{0} record(s) retrieved!" -f $Script:RunTimeData.QueryResult.d.Records | Write-LogOutput

                        $Script:MainFormForm.Elements.TextBlockStatusBarRows | Set-TextBlockText -Text ("{0:n0} rows" -f [Int]$Script:RunTimeData.QueryResult.d.Records)
                ...
                if ($null -ne $Script:RunTimeData.StopWatch) {
                    ...
                    $Script:MainFormForm.Elements.TextBlockStatusBarQueryTime.Text = $Script:RunTimeData.StopWatch.Elapsed.ToString()
                }
            }
            catch {
                $Script:MainFormForm.Elements.ButtonSaveQuery.IsEnabled = $true
