function Update-QueryList {
    ...
    try {
        ...
        if (($Script:RunTimeData.QueryListCache.QueryList | Measure-Object).Count -le 0) {
            ...
            # try/finally around everything after the message is written, matching
            # Complete-DataConnectionListUpdate. The reset used to sit on the success path only, and
            # this body has an early return (the ErrorRecord branch below) and an outer catch that
            # swallows - so a refresh that failed or threw left "Refreshing queries..." on the bar for
            # the rest of the session, where the user looks to find out whether they are connected.
            try {
