function Invoke-OmadaPSWebRequestWrapper {
    ...
    try {
        if (!$Script:RunTimeData.SkipRetryRequest) {
            ...
            $Private:Parameters.AuthenticationType = $($Script:MainFormForm.Elements.ComboBoxSelectAuthenticationOption.SelectedItem.Content)
            ...
            if ($null -ne $Script:MainFormForm -and $null -ne $Script:MainFormForm.Definitions -and $Script:MainFormForm.Definitions.IsVisible) {
                $Script:MainFormForm.Definitions.TextBlockStatusBarConnectionStatus | Set-TextBlockText -Text "Connected"
            }
