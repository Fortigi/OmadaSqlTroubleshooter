function Open-LogForm {
    ...
    try {
        ...
        $Script:LogForm = Initialize-FormObject -FormPath (Join-Path $Script:RunTimeConfig.ModuleFolder -ChildPath "lib\ui\LogWindow.xaml") -ParentForm $Script:MainFormForm.Definition
        ...
        if ($null -ne ($Script:LogForm.Definition | Get-WindowPositionConfig)) {
            $Position = $Script:LogForm.Definition | Get-WindowPositionConfig
