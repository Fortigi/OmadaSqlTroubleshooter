$Script:MainFormForm.Elements.ComboBoxSelectQuery.Add_DropDownOpened({
        try {
            $_ | Show-EventInfo

            # if (($Script:MainFormForm.Elements.ComboBoxSelectQuery.Items | Measure-Object).Count -lt 1) {
...
$Script:MainFormForm.Elements.ComboBoxSelectQuery.Add_SelectionChanged({
        try {
            $_ | Show-EventInfo

            $Script:RunTimeData.CurrentSqlQuery.FullName = $Script:MainFormForm.Elements.ComboBoxSelectQuery.SelectedItem.Content
            if (($Script:MainFormForm.Elements.ComboBoxSelectQuery.Items | Measure-Object).Count -ge 0 -and ![string]::IsNullOrWhiteSpace($Script:MainFormForm.Elements.ComboBoxSelectQuery.SelectedItem.Content)) {
                #Update-QueryList
                #Update-DataConnectionList
