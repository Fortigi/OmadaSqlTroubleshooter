#requires -Module OmadaWeb.PS
#requires -Version 7.0
[cmdletbinding()]
PARAM(
    [ValidateSet("INFO", "DEBUG", "VERBOSE", "WARNING", "ERROR", "FATAL")]
    [string]$LogLevel
)
$Error.Clear()

$ScriptName = "OmadaSqlTroubleshooter.ps1"
$ApplicationTitle = ""
$ScriptRootFolder = (Get-Item $PSScriptRoot).FullName
Push-Location $ScriptRootFolder

$StartVariables = Get-Variable

$ConfigFileName = $ScriptName -replace ".ps1", ".json"

$AppDataRootFolder = Join-Path $Env:APPDATA -ChildPath (Get-Item $ScriptRootFolder).Name

iF(Test-Path $AppDataRootFolder -PathType Container) {
    New-Item (Join-Path $AppDataRootFolder -ChildPath "Config") -ItemType Directory -Force | Out-Null
    $Script:ConfigFilePath = (Join-Path $AppDataRootFolder -ChildPath "Config\$ConfigFileName")
}
else {
    $Script:ConfigFilePath = Join-Path $ScriptRootFolder -ChildPath $ConfigFileName
}

$SqlQueryDoIdAttribute = "c-13"
$SqlQueryCreatedByAttribute = "c-2"
$SqlQueryChangedByAttribute = "c-4"
$Script:SqlTroubleShooterConfig = $null
$Script:AuthenticationNotSet = $false
$QueryText = $null
$SqlQueryObject = $null
$Script:CurrentSqlQueryDisplayName = $null
$Result = $null
$Script:QueryResult = $null
$Script:LogLevelSetting = [string]::IsNullOrWhiteSpace($LogLevel) ? $null : $LogLevel
$Script:OutputFileName = $null
$Script:CurrentQueryText = $null
$WebView = $null
$Script:WebviewUserDataFolder = $null

try {
    Remove-Variable "Task" -ErrorAction SilentlyContinue
}
catch {}

#region assemblies
Import-Module OmadaWeb.PS

#Set path to the bin folder to be sure that WebView2Loader.dll is found there.
$Env:Path += ";$ScriptRootFolder\Bin"
$Env:Path += ";$ScriptRootFolder"

Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing
Add-Type -AssemblyName PresentationFramework
Add-Type -AssemblyName WindowsBase
Add-Type -AssemblyName PresentationCore

"Microsoft.Web.WebView2.Core.dll", "Microsoft.Web.WebView2.Wpf.dll" | ForEach-Object {
    $WebView2Path = Join-Path $ScriptRootFolder -ChildPath "Bin\$_"
    if ((Test-Path $WebView2Path -PathType Leaf)) {
        [System.Reflection.Assembly]::LoadFrom($WebView2Path) | Out-Null
    }
    else {
        Throw ("The WebView2 Dll '{0}' is cannot be found at the '{1}' bin folder!" -f $_, $DllSource)
        Break
    }
}
$WebviewLoaderPath = Join-Path $ScriptRootFolder -ChildPath "Bin\WebView2Loader.dll"
if (!(Test-Path $WebviewLoaderPath -PathType Leaf)) {
    Throw ("The WebView2Loader Dll '{0}' is cannot be found at the '{1}' bin folder!" -f "WebView2Loader.dll", $DllSource)
    Break
}

#endregion

#region functions
function Get-Icon {
    PARAM(
        [ValidateSet("Wpf", "WinForms", "Base64")]
        [string]$Type = "WinForms"
    )

    # Base64 encoded icon (Create via $Base64Icon = [Convert]::ToBase64String([IO.File]::ReadAllBytes("Icon file path")))
    $Base64Icon = ""

    $IconBytes = [System.Convert]::FromBase64String($Base64Icon)

    $MemoryStream = New-Object System.IO.MemoryStream(, $IconBytes)

    switch ($Type) {
        "Wpf" {
            $Icon = New-Object System.Windows.Media.Imaging.BitmapImage
            $Icon.BeginInit()
            $Icon.StreamSource = $MemoryStream
            $Icon.CacheOption = [System.Windows.Media.Imaging.BitmapCacheOption]::OnLoad
            $Icon.EndInit()
            $Icon.Freeze()  # Freeze to make it thread-safe

            return $Icon
        }
        "WinForms" {
            return [System.Drawing.Icon]::new($MemoryStream)
        }
        Default {
            return $Base64Icon
        }
    }
}

function Write-LogOutput {
    param(
        [parameter(Mandatory = $True, Position = 0, ValueFromPipeline = $True)]
        [string]$Message,
        [ValidateSet("DEBUG", "INFO", "ERROR", "VERBOSE", "WARNING", "FATAL", "LOG")]
        [string]$LogType = "INFO"

    )

    $DateTime = Get-Date -Format "yyyy-MM-dd HH:mm:ss"

    $PSCallStack = Get-PSCallStack
    try {
        $Command = $null
        $Command = $PSCallStack[1]
        if ([string]::IsNullOrWhiteSpace($Command.Command)) {
            (Get-PSCallStack) | ForEach-Object {
                if ([string]::IsNullOrWhiteSpace($Command.Command) -and $_.Command -ne $MyInvocation.MyCommand -and ![string]::IsNullOrWhiteSpace($_.Command)) {
                    $Command = $_
                }
            }
        }
        $CalledFrom = "{0} ({1})" -f $Command.Command, $Command.ScriptLineNumber
    }
    catch {
        $CalledFrom = $null
    }
    $Message = "{0} - {1} - {2}: {3}`r`n" -f $DateTime, $LogType, $CalledFrom, $Message

    $WriteLogOutput = $false
    switch ($Script:LogLevelSetting) {
        "VERBOSE" {
            if ($LogType -in @( "INFO", "ERROR", "VERBOSE", "DEBUG", "WARNING", "FATAL", "LOG")) {
                $WriteLogOutput = $true
                $Message | Write-Verbose
            }
        }
        "DEBUG" {
            if ($LogType -in @( "INFO", "ERROR", "DEBUG", "WARNING", "FATAL", "LOG")) {
                $WriteLogOutput = $true
                $Message | Write-Verbose
            }
        }
        "INFO" {
            if ($LogType -in @( "INFO", "ERROR", "WARNING", "FATAL", "LOG")) {
                $WriteLogOutput = $true
            }
        }

        "WARNING" {
            if ($LogType -in @( "ERROR", "WARNING", "FATAL", "LOG")) {
                $WriteLogOutput = $true
            }
        }
        { "ERROR", "FATAL" } {
            if ($LogType -in @("ERROR", "FATAL")) {
                $WriteLogOutput = $true
                try {
                    $CallStack = $null # Get-PSCallStack | ConvertTo-Json -Depth 15 -ErrorAction SilentlyContinue
                    "{0}`r`n{1}" -f $Message, $CallStack | Write-Verbose
                }
                catch {}
            }
            elseif ($LogType -eq "LOG") {
                $WriteLogOutput = $true
            }
        }
        Default {
            $WriteLogOutput = $false
        }
    }

    if ($WriteLogOutput) {
        $TextBoxOutput.AppendText($Message)
    }
    $TextBoxOutput.ScrollToEnd()
}

function Invoke-ProcessConfigSettings {
    PARAM(
        [parameter(Mandatory = $false, Position = 0, ValueFromPipeline = $true)]
        $Value,
        [parameter(Mandatory = $false)]
        [string]$Property
    )


    $ConfigProperties = @("BaseUrl", "SqlQueryDoId", "SelectedSqlQueryDoId", "LastOutputFolder", "LastExtension", "LastAuthentication", "UserName", "LogLevel", "MyQueriesOnly", "IdentityUserName", "CurrentDataConnection", "CurrentDataConnectionId", "CurrentDataConnectionName")
    if ($Null -ne $Script:SqlTroubleShooterConfig) {
        $Config = $Script:SqlTroubleShooterConfig | ConvertTo-Json | ConvertFrom-Json
        $CurrentPoperties = $Config | Get-Member -MemberType NoteProperty
        $ConfigProperties | ForEach-Object {
            if ($_ -notin $CurrentPoperties.Name) {
                "Add property {0} to config object!" -f $_ | Write-LogOutput -LogType VERBOSE
                $Value = $Null
                if ($_ -eq "LogLevel") {
                    $Value = "INFO"
                }
                if ($_ -eq "LastAuthentication") {
                    $Value = "Browser"
                }
                $Config | Add-Member -MemberType NoteProperty -Name $_ -Value $Value
            }
        }
        "Update config object!" | Write-LogOutput -LogType VERBOSE
    }
    else {
        if (Test-Path $Script:ConfigFilePath) {
            "Read config settings {0}!" -f $Script:ConfigFilePath | Write-LogOutput -LogType VERBOSE
            $Config = Get-Content $Script:ConfigFilePath | ConvertFrom-Json
            $CurrentPoperties = $Config | Get-Member -MemberType NoteProperty
            $ConfigProperties | ForEach-Object {
                if ($_ -notin $CurrentPoperties.Name) {
                    "Add property {0} to config object!" -f $_ | Write-LogOutput -LogType VERBOSE
                    $Config | Add-Member -MemberType NoteProperty -Name $_ -Value $Null
                }
            }
        }
        else {
            "Create new config object!" | Write-LogOutput -LogType DEBUG
            $Config = [pscustomobject]@{}
            $ConfigProperties | ForEach-Object {
                "Add property {0} to config object!" -f $_ | Write-LogOutput -LogType VERBOSE
                $Config | Add-Member -MemberType NoteProperty -Name $_ -Value $Null
            }
        }
    }

    if ($null -ne $Value -and $null -ne $Property) {
        "Add value for property {0} in config object!" -f $_ | Write-LogOutput -LogType VERBOSE
        $Config.$Property = $Value
    }

    "Store config object to {0}. Contents`r`n{1}!" -f $Script:ConfigFilePath, ($Config | ConvertTo-Json) | Write-LogOutput -LogType VERBOSE
    $Config | ConvertTo-Json | Set-Content $Script:ConfigFilePath -Force
    $Script:SqlTroubleShooterConfig = $Config
}

function Set-AuthenticationOption {
    if ($Null -ne $ComboBoxSelectAuthenticationOption.SelectedItem.Content) {

        switch ($ComboBoxSelectAuthenticationOption.SelectedItem.Content) {
            { $_ -in @("Basic", "Windows", "OAuth" ) } {

                if ($_ -eq "OAuth") {
                    $LabelUserName.Content = "Client ID:"
                    $LabelPassword.Content = "Client Secret:"
                }
                else {
                    $LabelUserName.Content = "Username:"
                    $LabelPassword.Content = "Password:"
                }
                $LabelUserName.Visibility = "Visible"
                $LabelPassword.Visibility = "Visible"
                $TextBoxUserName.Visibility = "Visible"
                $TextBoxPassword.Visibility = "Visible"
                $TextBoxUserName.IsEnabled = $True
                $TextBoxPassword.IsEnabled = $True
                if (!$InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    $InvokeOmadaRestMethodParam.Add("Credential", $Null)
                }
            }

            default {
                $TextBoxUserName.IsEnabled = $False
                $TextBoxPassword.IsEnabled = $False
                $LabelUserName.Visibility = "Hidden"
                $LabelPassword.Visibility = "Hidden"
                $TextBoxUserName.Visibility = "Hidden"
                $TextBoxPassword.Visibility = "Hidden"
                $TextBoxUserName.Text = ""
                $TextBoxPassword.Password = ""
                $Null | Invoke-ProcessConfigSettings -Property "UserName"
                if ($InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    $InvokeOmadaRestMethodParam.Remove("Credential")
                }
            }
        }
        $ComboBoxSelectAuthenticationOption.SelectedItem.Content | Invoke-ProcessConfigSettings -Property "LastAuthentication"
    }
}


function Invoke-OmadaPSWebRequestWrapper {
    try {
        $InvokeOmadaRestMethodParam.Uri = $QueryUrl
        $InvokeOmadaRestMethodParam.Method = $Method
        $InvokeOmadaRestMethodParam.AuthenticationType = $($ComboBoxSelectAuthenticationOption.SelectedItem.Content)
        if ($Null -eq $Body) {
            if ($InvokeOmadaRestMethodParam.ContainsKey("Body")) {
                $InvokeOmadaRestMethodParam.Remove("Body")
            }
        }
        else {
            if (!$InvokeOmadaRestMethodParam.ContainsKey("Body")) {
                $InvokeOmadaRestMethodParam.Add("Body", $Null)
            }

            $InvokeOmadaRestMethodParam.Body = $Body | ConvertTo-Json
        }
        $Result = Invoke-OmadaRestMethod @Script:InvokeOmadaRestMethodParam
        "Result: {0}" -f ($Result | ConvertTo-Json -Depth 15) | Write-LogOutput -LogType VERBOSE
        return $Result
    }
    catch {
        if (![string]::IsNullOrWhiteSpace($_.ErrorDetails?.Message)) {
            $Message = $_.ErrorDetails.Message
            try {
                $MessageObject = $Message | ConvertFrom-Json

                $Message = (($MessageObject | Get-Member -MemberType NoteProperty).Name | ForEach-Object { "{0}: {1}" -f $_, $MessageObject.$_ }) -join "`r`n"
            }
            catch {}
        }
        else {
            $Message = $_.Exception.Message
        }
        $Message | Write-LogOutput -LogType ERROR
        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}

function Set-OmadaUrl {

    if (![string]::IsNullOrWhiteSpace($TextBoxURL.Text)) {

        if ($TextBoxURL.Text -notlike "http*") {
            if ($TextBoxURL.Text -notlike "*.*" -and $TextBoxURL.Text -notlike "*.omada.cloud") {
                $TextBoxURL.Text = "https://$($TextBoxURL.Text).omada.cloud"
            }
            else {
                $TextBoxURL.Text = "https://$($TextBoxURL.Text)"
            }
        }

        $Uri = [System.Uri]::new($TextBoxURL.Text.Trim())

        if ($Uri.IsAbsoluteUri -and ($Uri.Scheme -eq 'http' -or $Uri.Scheme -eq 'https')) {
        ("Input Url {0} is valid." -f $TextBoxURL.Text.Trim()) | Write-LogOutput -LogType DEBUG
        }
        else {
            $Null | Invoke-ProcessConfigSettings -Property "BaseUrl"
            $TextBoxURL.Text = $Null
            $Message = "Input Url {0} is not valid." -f $TextBoxURL.Text.Trim()
            $Message | Write-LogOutput -LogType ERROR
            [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            return
        }
        try {
            $DnsResult = Resolve-DnsName -Name $Uri.Host -QuickTimeout -ErrorAction SilentlyContinue
            if (($DnsResult | Measure-Object).Count -le 0) {
                $Message = "DNS resolution for {0} failed!" -f $Uri.Host
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                return
            }
        }
        catch {
            $Null | Invoke-ProcessConfigSettings -Property "BaseUrl"
            $TextBoxURL.Text = $Null
            $Message = "Endpoint {0} not found!" -f $Uri.AbsoluteUri
            $Message | Write-LogOutput -LogType ERROR
            [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        }
        $Uri.AbsoluteUri.TrimEnd("/") | Invoke-ProcessConfigSettings -Property "BaseUrl"

        if ($CurrentUrl -ne $Script:SqlTroubleShooterConfig.BaseUrl -or $Script:AuthenticationNotSet) {
            "Omada Url set to: {0}" -f $Script:SqlTroubleShooterConfig.BaseUrl | Write-LogOutput -LogType DEBUG
            if ($null -ne $ComboBoxSelectAuthenticationOption.SelectedItem.Content) {
                Update-QueryList
                $ComboBoxSelectQuery.IsEnabled = $True
                $ButtonRefreshQueries.IsEnabled = $True
            }
        }
    }
    else {
        $ComboBoxSelectQuery.IsEnabled = $False
        $ComboBoxSelectQuery.SelectedItem = $Null
        $ButtonRefreshQueries.IsEnabled = $False
        "Omada Url is empty!" | Write-LogOutput -LogType DEBUG
    }

}

function Get-OmadaGetPagingDataObject {
    PARAM(
        [parameter(Mandatory = $True, Position = 0)]
        [string]$DataType,
        [parameter(Mandatory = $True, Position = 1)]
        [hashtable]$DataTypeArgs,
        [parameter(Mandatory = $False, Position = 3)]
        [string]$SearchString = $null,
        [parameter(Mandatory = $false, Position = 4)]
        [int]$Rows = 1000
    )

    $Body = [ordered]@{
        _search      = $false
        nd           = 1732546553116
        rows         = $Rows
        page         = 1
        sidx         = [string]::IsNullOrWhiteSpace($SearchString) ? $null : "name"
        sord         = "asc"
        searchField  = $null
        searchString = [string]::IsNullOrWhiteSpace($SearchString) ? $null : $SearchString
        searchOper   = $null
        filters      = $null
        dataType     = $DataType
        dataTypeArgs = $DataTypeArgs
    }

    $QueryUrl = '{0}/WebService/JQGridPopulationWebService.asmx/GetPagingData' -f $Script:SqlTroubleShooterConfig.BaseUrl
    $Method = "POST"

    return Invoke-OmadaPSWebRequestWrapper
}

function Get-SqlTroubleShooterView {

    $ViewResult = Get-OmadaGetPagingDataObject -SearchString "SQL Troubleshooting" -DataType "Views" -DataTypeArgs @{OwnerShipType = "Both" }
    $View = $null
    if ($null -ne $ViewResult -and $ViewResult.d.Records -gt 0) {
        $View = $ViewResult.d.Rows | Where-Object { $_.Name -eq "SQL Troubleshooting" }
    }
    $Result = $null
    if ($null -ne $View) {
        $DataTypeArgs = [ordered]@{
            viewId          = ("{0}" -f $View.Id)
            pageQueryString = ("{0}/dataobjlst.aspx?view={1}" -f $Script:SqlTroubleShooterConfig.BaseUrl, $View.Id)
            readOnlyMode    = $false
            countRows       = $false
        }

        $Result = Get-OmadaGetPagingDataObject -DataType "DataObjects" -DataTypeArgs $DataTypeArgs
        $Result = $Result.d.Rows
    }
    return $Result

}

#Todo: Validate if this function is working fast enough.
# function Update-QueryList {

#     $SqlQueryViewContents = Get-SqlTroubleShooterView | Where-Object { $_.$($SqlQueryCreatedByAttribute) -eq $Script:SqlTroubleShooterConfig.IdentityUserName -or $_.$($SqlQueryChangedByAttribute) -eq $Script:SqlTroubleShooterConfig.IdentityUserName }

#     $QueryUrl = '{0}/odata/dataobjects/C_P_SQLTROUBLESHOOTING?$filter=Deleted ne true and NAME ne ''''' -f $Script:SqlTroubleShooterConfig.BaseUrl
#     "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG
#     "Refresh queries started" | Write-LogOutput
#     $Body = $Null
#     $Method = "GET"
#     $Result = Invoke-OmadaPSWebRequestWrapper

#     $Result.value | Sort-Object DisplayName | ForEach-Object {
#         $DOIDDisplayName = "{0} - {1}" -f $_.Id, $_.DisplayName
#         if($Script:SqlTroubleShooterConfig.MyQueriesOnly -and $null -ne $SqlQueryViewContents -and $_.Id -notin $SqlQueryViewContents.$SqlQueryDoIdAttribute) {
#             "Skip query {0} - {1} because of 'Filter My Queries Only' is enabled" -f $_.Id, $_.DisplayName | Write-LogOutput -LogType DEBUG
#         }
#         else {
#             if($DOIDDisplayName -notin $ComboBoxSelectQuery.Items.Content -and $ComboBoxSelectQuery.SelectedItem.Content -ne $DOIDDisplayName) {
#                 "Add query {0} - {1}!" -f $_.Id, $_.DisplayName | Write-LogOutput -LogType DEBUG
#                 $ComboBoxSelectQueryItem = New-Object System.Windows.Controls.ComboBoxItem
#                 $ComboBoxSelectQueryItem.Content = $DOIDDisplayName
#                 $ComboBoxSelectQueryItem.Background = [System.Windows.Media.Brushes]::White
#                 $ComboBoxSelectQuery.Items.Add($ComboBoxSelectQueryItem) | Out-Null
#             }
#             if($DOIDDisplayName -in $ComboBoxSelectQuery.Items -or $ComboBoxSelectQuery.SelectedItem -eq $DOIDDisplayName) {
#                 if($null -ne $ComboBoxSelectQueryItem) {
#                     $ComboBoxSelectQueryItem.SelectedItem = $ComboBoxSelectQueryItem
#                 }
#                 $ComboBoxSelectQuery.Items.Remove($DOIDDisplayName)
#             }
#             if($null -ne $SqlQueryViewContents -and $_.Id -in $SqlQueryViewContents.$SqlQueryDoIdAttribute) {
#                 "Update query {0} - {1}!" -f $_.Id, $_.DisplayName | Write-LogOutput -LogType DEBUG
#                 $ComboBoxSelectQueryItem = $null
#                 $ComboBoxSelectQueryItem = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $DOIDDisplayName }
#                 if($null -ne $ComboBoxSelectQueryItem) {
#                     $ComboBoxSelectQueryItem.Background = [System.Windows.Media.Brushes]::LightGray
#                 }
#             }
#         }
#         $ComboBoxSelectQueryItem = $null
#     }
#     #$SelectedItem = $ComboBoxSelectQuery.SelectedItem
#     #$ComboBoxSelectQuery.Items | Sort-Object { ($_.Content -split " - ")[1].Trim()} | ForEach-Object {
#     #    $ComboBoxSelectQuery.Items.Remove($_)
#     #    $ComboBoxSelectQuery.Items.Add($_)
#     #}
#     #$Comboboxselectquery.SelectedItem = $SelectedItem

#     $ComboBoxSelectQuery.IsEnabled = $True
#     $ButtonRefreshQueries.IsEnabled = $True
#     "{0} queries retrieved!" -f ($Result.Value | Measure-Object).Count | Write-LogOutput
# }

function Update-QueryList {

    if ($Script:SqlTroubleShooterConfig.MyQueriesOnly -and ![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.IdentityUserName)) {
        $SqlQueryViewContents = Get-SqlTroubleShooterView | Where-Object { $_.$($SqlQueryCreatedByAttribute) -eq $Script:SqlTroubleShooterConfig.IdentityUserName -or $_.$($SqlQueryChangedByAttribute) -eq $Script:SqlTroubleShooterConfig.IdentityUserName }
    }

    $QueryUrl = '{0}/odata/dataobjects/C_P_SQLTROUBLESHOOTING?$filter=Deleted ne true and NAME ne ''''' -f $Script:SqlTroubleShooterConfig.BaseUrl
    "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG
    "Refresh queries started" | Write-LogOutput
    $Body = $Null
    $Method = "GET"
    $Body = $null
    $Result = Invoke-OmadaPSWebRequestWrapper

    $Result.value | ForEach-Object {
        $DOIDDisplayName = "{0} - {1}" -f $_.Id, $_.DisplayName
        if ($Script:SqlTroubleShooterConfig.MyQueriesOnly -and $null -ne $SqlQueryViewContents -and $_.Id -notin $SqlQueryViewContents.$SqlQueryDoIdAttribute) {
            "Skip query {0} - {1} because of 'Filter My Queries Only' is enabled" -f $_.Id, $_.DisplayName | Write-LogOutput -LogType DEBUG

            if ($DOIDDisplayName -in $ComboBoxSelectQuery.Items.Content) {
                $ComboBoxSelectQueryItem = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $DOIDDisplayName }
                $ComboBoxSelectQuery.Items.Remove($ComboBoxSelectQueryItem) | Out-Null
            }
        }
        else {
            $DOIDDisplayName = "{0} - {1}" -f $_.Id, $_.DisplayName
            if ($DOIDDisplayName -notin $ComboBoxSelectQuery.Items.Content) {
                $ComboBoxSelectQueryItem = New-Object System.Windows.Controls.ComboBoxItem
                $ComboBoxSelectQueryItem.Content = $DOIDDisplayName
                $ComboBoxSelectQuery.Items.Add($ComboBoxSelectQueryItem) | Out-Null
            }
        }
    }
    $ComboBoxSelectQuery.IsEnabled = $True
    $ButtonRefreshQueries.IsEnabled = $True
    "{0} queries retrieved!" -f ($Result.Value | Measure-Object).Count | Write-LogOutput
}

function Invoke-ExecuteScriptWithResultAsync {
    PARAM(
        $ScriptToExecute,
        $OnCompletedScriptBlock
    )

    if ($null -ne $WebView.CoreWebView2) {
        $Script:Task = $WebView.CoreWebView2.ExecuteScriptWithResultAsync($ScriptToExecute)

        $Script:Task.GetAwaiter().OnCompleted($OnCompletedScriptBlock)
    }
    else {
        Write-LogOutput -Message "WebView2 is not initialized." -LogType ERROR
    }
}

function Invoke-ExecuteScriptAsync {
    PARAM(
        $ScriptToExecute,
        $OnCompletedScriptBlock
    )

    if ($null -ne $WebView.CoreWebView2) {
        $Script:Task = $WebView.CoreWebView2.ExecuteScriptAsync($ScriptToExecute)
        $Script:Task.GetAwaiter().OnCompleted($OnCompletedScriptBlock)
    }
    else {
        Write-LogOutput -Message "WebView2 is not initialized." -LogType ERROR
    }
}

function Set-EditorBackground {

    $OnCompletedScriptBlock = {
        try {
            if (!$Script:Task.Status -eq "RanToCompletion") {
                $Message = "Task failed: {0}" -f $Script:Task.Status
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        }
        catch {
            $Message = "Error: {0}" -f $Script:Task.Exception.Message
            $Message | Write-LogOutput -LogType ERROR
            [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
        }
    }
    $ScriptToExecute = "container.style.backgroundImage = url('`${0}');" -f (Get-Icon -Type Base64)
    Invoke-ExecuteScriptAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
    $ScriptToExecute = "container.style.backgroundSize = `"200px 200px`";"
    Invoke-ExecuteScriptAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
    $ScriptToExecute = "container.style.backgroundPosition = `"center`";"
    Invoke-ExecuteScriptAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
    $ScriptToExecute = "container.style.backgroundRepeat = `"no-repeat`";"
    Invoke-ExecuteScriptAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
}

#TODO: Merge with Invoke-SaveAndExecuteQuery
function Invoke-SaveQuery {

    try {
        $ScriptToExecute = "editor.getValue();"

        $OnCompletedScriptBlock = {
            try {
                if ($Script:Task.Status -eq "RanToCompletion") {
                    $QueryText = $Script:Task.Result
                    if (![string]::IsNullOrWhiteSpace($QueryText.ResultAsJson)) {
                        $QueryText = $QueryText.ResultAsJson | ConvertFrom-Json
                    }

                    $Result = Get-SqlQueryObject

                    $Body = @{}
                    $QueryUrl = "{0}/odata/dataobjects/C_P_SQLTROUBLESHOOTING({1})" -f $Script:SqlTroubleShooterConfig.BaseUrl, $Script:SqlTroubleShooterConfig.SqlQueryDoId
                    if ($Script:CurrentQueryText -ne $QueryText -or $QueryText -ne $Result.C_QUERY) {
                        "Update current query for DODI: {0}" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId | Write-LogOutput -LogType DEBUG
                        $Body.Add("C_QUERY", $QueryText)
                        if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.CurrentDataConnectionId)) {
                            $Body.Add("C_SQLTROUBLESHOOTING_DATACONNECTION", @{Id = $Script:SqlTroubleShooterConfig.CurrentDataConnectionId })
                        }
                    }
                    if ($Script:CurrentSqlQueryDisplayName -ne $TextBoxDisplayName.Text) {
                        $Body.Add("NAME", $TextBoxDisplayName.Text)
                    }
                    if (($Body.Keys | Measure-Object).Count -le 0) {
                        "No changes detected! Saving not needed." | Write-LogOutput -LogType DEBUG
                    }
                    else {
                        "Saving SQL Query: {0}" -f $QueryText | Write-LogOutput -LogType DEBUG
                        "Body: {0}" -f ($Body | ConvertTo-Json) | Write-LogOutput -LogType VERBOSE
                        "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG

                        "Save query" | Write-LogOutput
                        $Method = "PUT"
                        $Result = Invoke-OmadaPSWebRequestWrapper
                        "Query saved!" | Write-LogOutput

                        "{0} - {1}" -f $Result.Id, $Result.DisplayName | Invoke-ProcessConfigSettings -Property "SelectedSqlQueryDoId"
                        if ($Result.DisplayName -ne $Script:CurrentSqlQueryDisplayName) {
                            "New display name, Current: {0}, New: {1}" -f $Script:CurrentSqlQueryDisplayName, $Result.DisplayName | Write-LogOutput -LogType VERBOSE
                            Update-QueryList
                            $ComboBoxSelectQueryItem = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId }
                            if ($null -ne $ComboBoxSelectQueryItem) {
                                $ComboBoxSelectQueryItem = New-Object System.Windows.Controls.ComboBoxItem
                                $ComboBoxSelectQueryItem.Content = $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId
                                $ComboBoxSelectQuery.Items.Add($ComboBoxSelectQueryItem) | Out-Null
                            }
                            $ComboBoxSelectQuery.SelectedItem = $ComboBoxSelectQueryItem
                            $Script:CurrentSqlQueryDisplayName = $Result.DisplayName
                        }
                    }
                }
                elseif ($Script:Task.Status -eq "Faulted") {
                    $Message = "Task failed: {0}" -f $Script:Task.Status
                    $Message | Write-LogOutput -LogType ERROR
                    [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                }
                else {
                    $Message = "Task result: {0}" -f $Script:Task.Status
                    $Message | Write-LogOutput -LogType DEBUG
                }
                $ButtonSaveQuery.IsEnabled = $True
                $ButtonExecuteQuery.IsEnabled = $True
            }
            catch {
                $Message = "Error: {0}" -f $Script:Task.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        }
        Invoke-ExecuteScriptWithResultAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
    }
    catch {
        $Message = $_.Exception.Message
        $Message | Write-LogOutput -LogType ERROR
        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}

#TODO: Merge with Invoke-SaveQuery
function Invoke-SaveAndExecuteQuery {

    try {
        $ScriptToExecute = "editor.getValue();"

        $OnCompletedScriptBlock = {
            try {
                if ($Script:Task.Status -eq "RanToCompletion") {
                    $QueryText = $Script:Task.Result
                    if (![string]::IsNullOrWhiteSpace($QueryText.ResultAsJson)) {
                        $QueryText = $QueryText.ResultAsJson | ConvertFrom-Json
                    }

                    $Result = Get-SqlQueryObject

                    "Executing SQL Query: {0}" -f $QueryText | Write-LogOutput -LogType DEBUG
                    $Body = @{}
                    $QueryUrl = "{0}/odata/dataobjects/C_P_SQLTROUBLESHOOTING({1})" -f $Script:SqlTroubleShooterConfig.BaseUrl, $Script:SqlTroubleShooterConfig.SqlQueryDoId
                    if ($Script:CurrentQueryText -ne $QueryText -or $QueryText -ne $Result.C_QUERY) {
                        "Update current query for DODI: {0}" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId | Write-LogOutput -LogType DEBUG
                        $Body.Add("C_QUERY", $QueryText)
                        if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.CurrentDataConnectionId)) {
                            $Body.Add("C_SQLTROUBLESHOOTING_DATACONNECTION", @{Id = $Script:SqlTroubleShooterConfig.CurrentDataConnectionId })
                        }
                    }
                    if ($Script:CurrentSqlQueryDisplayName -ne $TextBoxDisplayName.Text) {
                        $Body.Add("NAME", $TextBoxDisplayName.Text)
                    }
                    if (($Body.Keys | Measure-Object).Count -le 0) {
                        "No changes detected! Just run query" | Write-LogOutput -LogType DEBUG
                    }
                    else {
                        "Body: {0}" -f ($Body | ConvertTo-Json) | Write-LogOutput -LogType VERBOSE
                        "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG

                        "Save query" | Write-LogOutput
                        $Method = "PUT"
                        $Result = Invoke-OmadaPSWebRequestWrapper
                        "Query saved!" | Write-LogOutput
                    }
                    $QueryUrl = "{0}/webservice/jQGridPopulationWebService.asmx/GetPagingData" -f $Script:SqlTroubleShooterConfig.BaseUrl

                    $Body = @{
                        "dataType"     = "SqlDataProducer"
                        "dataTypeArgs" = @{
                            "targetId" = $Script:SqlTroubleShooterConfig.SqlQueryDoId
                        }
                        "page"         = 1
                        "rows"         = 100000
                        "sidx"         = $Null
                        "sord"         = "asc"
                        "_search"      = $False
                        "searchField"  = $Null
                        "searchString" = $Null
                        "filters"      = $Null
                        "searchOper"   = $Null
                    }
                    "Body: {0}" -f ($Body | ConvertTo-Json) | Write-LogOutput -LogType VERBOSE
                    "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG

                    "Retrieve query output, please wait..." | Write-LogOutput
                    $Method = "POST"
                    $Script:QueryResult = $null
                    $Script:QueryResult = Invoke-OmadaPSWebRequestWrapper


                    if ($null -ne $Script:QueryResult -and ($Script:QueryResult.d.Rows | Measure-Object).Count -le 0) {
                        "Query did not return any results!" | Write-LogOutput -LogType WARNING
                    }
                    else {
                        "Result:`r`n{0}" -f ($Script:QueryResult.d.rows | Format-Table -AutoSize | Out-String -Width 10000000 ) | Write-LogOutput
                        $ButtonShowOutput.IsEnabled = $True
                        $ButtonSaveOutputFile.IsEnabled = $True
                        "{0} record(s) retrieved!" -f $Script:QueryResult.d.Records | Write-LogOutput

                        "{0} - {1}" -f $Result.Id, $Result.DisplayName | Invoke-ProcessConfigSettings -Property "SelectedSqlQueryDoId"
                        if ($Result.DisplayName -ne $Script:CurrentSqlQueryDisplayName) {
                            "New display name, Current: {0}, New: {1}" -f $Script:CurrentSqlQueryDisplayName, $Result.DisplayName | Write-LogOutput -LogType DEBUG
                            Update-QueryList
                            $ComboBoxSelectQueryItem = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId }
                            if ($null -ne $ComboBoxSelectQueryItem) {
                                $ComboBoxSelectQueryItem = New-Object System.Windows.Controls.ComboBoxItem
                                $ComboBoxSelectQueryItem.Content = $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId
                                $ComboBoxSelectQuery.Items.Add($ComboBoxSelectQueryItem) | Out-Null
                            }
                            $ComboBoxSelectQuery.SelectedItem = $ComboBoxSelectQueryItem
                        }
                    }
                }
                elseif ($Script:Task.Status -eq "Faulted") {
                    $Message = "Task failed: {0}" -f $Script:Task.Status
                    $Message | Write-LogOutput -LogType ERROR
                    [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                }
                else {
                    $Message = "Task result: {0}" -f $Script:Task.Status
                    $Message | Write-LogOutput -LogType DEBUG
                }
                $ButtonSaveQuery.IsEnabled = $True
                $ButtonExecuteQuery.IsEnabled = $True
            }
            catch {
                $Message = "Error: {0}" -f $_.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        }
        Invoke-ExecuteScriptWithResultAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
    }
    catch {
        $Message = $_.Exception.Message
        $Message | Write-LogOutput -LogType ERROR
        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
    }
}

function Get-SqlQueryObject {

    if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.SqlQueryDoId)) {
        $TextBoxURL.Text.Trim() | Invoke-ProcessConfigSettings -Property "BaseUrl"

        [int32]::Parse($Script:SqlTroubleShooterConfig.SqlQueryDoId) | Invoke-ProcessConfigSettings -Property "SqlQueryDoId"

        "Retrieve current query for SqlQuery DoId: {0}" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId | Write-LogOutput -LogType DEBUG
        $QueryUrl = "{0}/odata/dataobjects/C_P_SQLTROUBLESHOOTING({1})" -f $Script:SqlTroubleShooterConfig.BaseUrl, $Script:SqlTroubleShooterConfig.SqlQueryDoId
        "QueryUrl: {0}" -f $QueryUrl | Write-LogOutput -LogType DEBUG

        "Retrieve query {0}" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId | Write-LogOutput

        $Body = $Null
        $Method = "GET"
        return Invoke-OmadaPSWebRequestWrapper

        "Retrieved object {0}" -f $SqlQueryObject | Write-LogOutput -LogType VERBOSE
    }
    else {
        "SqlQueryDoId is not set! Cannot retrieve Sql query!" | Write-LogOutput -LogType WARNING
    }
}

function Set-EditorValue {
    if ($null -ne $ComboBoxSelectQuery.SelectedItem) {
        "Selected SQL Query object: {0}" -f $ComboBoxSelectQuery.SelectedItem.Content | Write-LogOutput -LogType DEBUG
        $ComboBoxSelectQuery.SelectedItem.Content.Split(" - ")[0].Trim() | Invoke-ProcessConfigSettings -Property "SqlQueryDoId"
        $ComboBoxSelectQuery.SelectedItem.Content | Invoke-ProcessConfigSettings -Property "SelectedSqlQueryDoId"

        if ([string]::IsNullOrWhiteSpace($TextBoxURL.Text) -or [string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.SqlQueryDoId)) {
            $Message = "Omada Url not set or Query not selected. Set correct values to execute queries!"
            $Message | Write-LogOutput -LogType WARNING
            [System.Windows.Forms.MessageBox]::Show(("Warning:`r`n`r`n{0}" -f $Message), "Warning", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Warning)
        }
        else {

            if ($InvokeOmadaRestMethodParam.ContainsKey("Credential") -and $Null -eq $InvokeOmadaRestMethodParam.Credential) {
                $Message = "Credential is not present, please check credential input!"
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }

            $Result = Get-SqlQueryObject

            $Script:CurrentSqlQueryDisplayName = $Result.DisplayName
            $TextBoxDisplayName.Text = $Result.DisplayName
            "{0} - {1}" -f $Result.C_SQLTROUBLESHOOTING_DATACONNECTION.Id, $Result.C_SQLTROUBLESHOOTING_DATACONNECTION.DisplayName | Invoke-ProcessConfigSettings -Property "CurrentDataConnection"
            $Result.C_SQLTROUBLESHOOTING_DATACONNECTION.Id | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionId"
            $Result.C_SQLTROUBLESHOOTING_DATACONNECTION.DisplayName | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionName"
            Set-DataConnection

            $ButtonExecuteQuery.IsEnabled = $True
            $ButtonSaveQuery.IsEnabled = $True
            $ButtonReset.IsEnabled = $True
            $TextBoxDisplayName.IsEnabled = $True


            if ($null -ne $WebView.CoreWebView2) {

                $ScriptToExecute = "editor.setValue('{0}');" -f ($Result.C_QUERY -replace "`n", "\n" -replace "`r", "\r" -replace "`t", "\t" -replace "'", "\'")

                $OnCompletedScriptBlock = {
                    try {
                        if ($Script:Task.Status -eq "RanToCompletion") {
                            "Editor value updated!" | Write-LogOutput -LogType DEBUG
                        }
                        elseif ($Script:Task.Status -eq "Faulted") {
                            $Message = "Task failed: {0}" -f $Script:Task.Status
                            $Message | Write-LogOutput -LogType ERROR
                            [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                        }
                        else {
                            $Message = "Task result: {0}" -f $Script:Task.Status
                            $Message | Write-LogOutput -LogType DEBUG
                        }
                    }
                    catch {
                        $Message = "Error: {0}" -f $Script:Task.Exception.Message
                        $Message | Write-LogOutput -LogType ERROR
                        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    }
                }

                Invoke-ExecuteScriptAsync -ScriptToExecute $ScriptToExecute -OnCompletedScriptBlock $OnCompletedScriptBlock
                $Script:CurrentQueryText = $Result.C_QUERY
                "Query {0} retrieved!" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId | Write-LogOutput
            }
        }
    }
}

function Set-DataConnection {

    if (!$ComboBoxSelectDataConnection.Items.Contains($Script:SqlTroubleShooterConfig.CurrentDataConnection)) {
        $ComboBoxSelectDataConnection.Items.Add($Script:SqlTroubleShooterConfig.CurrentDataConnection) | Out-Null
        $Script:SqlTroubleShooterConfig.CurrentDataConnection.Split(" - ")[0].Trim() | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionId"
        $Script:SqlTroubleShooterConfig.CurrentDataConnection.Split(" - ")[1].Trim() | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionName"
    }
    $ComboBoxSelectDataConnection.SelectedValue = $Script:SqlTroubleShooterConfig.CurrentDataConnection
}

function Update-DataConnectionList {
    "Retrieve data connections" | Write-LogOutput -LogType DEBUG
    $SqlQueryViewContents = Get-SqlTroubleShooterView
    if ($null -ne $SqlQueryViewContents) {
        $QueryUrl = "{0}/dataobjdlg.aspx?DOID={1}" -f $Script:SqlTroubleShooterConfig.BaseUrl, $SqlQueryViewContents[0].$SqlQueryDoIdAttribute
        $Body = $null
        $Method = "GET"
        $Result = Invoke-OmadaPSWebRequestWrapper

        if ($null -eq $Result) {
            "Failed to retrieve data connections! Data connection cannot be changed!" | Write-LogOutput -LogType WARNING
            $ComboBoxSelectDataConnection.IsEnabled = $False
        }
        else {
            $ComboBoxSelectDataConnection.IsEnabled = $True
            if ($null -ne $Result) {
                $Result -split "`r`n" | ForEach-Object {
                    $Options = [regex]::Matches($Result, '<option.*?value="(\d+).*?data-uid="(.*?)".*?>(.*?)</option>')
                    foreach ($Match in $Options) {
                        $DataConnectionDisplayName = "{0} - {1}" -f $Match.Groups[1].Value, $Match.Groups[3].Value
                        if ($DataConnectionDisplayName -notin $ComboBoxSelectDataConnection.Items) {
                            "Add data connection {0}" -f $DataConnectionDisplayName | Write-LogOutput -LogType DEBUG
                            $ComboBoxSelectDataConnection.Items.Add($DataConnectionDisplayName) | Out-Null
                        }
                    }
                }
            }
            "{0} data connections processed!" -f ($ComboBoxSelectDataConnection.Items | Measure-Object).Count | Write-LogOutput
        }
    }
}

function Show-EventInfo {
    PARAM(
        [parameter(Mandatory = $True, Position = 0, ValueFromPipeline = $True)]
        $Item
    )
    if ($Item -is [Microsoft.Web.WebView2.Core.CoreWebView2NavigationCompletedEventArgs]) {
        "Webview success: '{0}'" -f $Item.IsSuccess | Write-LogOutput -LogType DEBUG
    }
    elseif ($Item -is [System.Windows.Window]) {
        "Form: '{0}', Event: '{1}', Event Type: '{2}'" -f $Item.Source.Title, $Item.RoutedEvent.Name, $Item.RoutedEvent.OwnerType.FullName | Write-LogOutput -LogType DEBUG
    }
    elseif ($Item -is [System.Windows.Controls.SelectionChangedEventArgs]) {
        "Control: '{0}', Event: '{1}', Event Type: '{2}', Added values: {3}, Removed values: {4}" -f $Item.Source.Name, $Item.RoutedEvent.Name, $Item.RoutedEvent.OwnerType.FullName, ($Item.AddedItems | Measure-Object).Count, ($Item.RemovedItems | Measure-Object).Count | Write-LogOutput -LogType DEBUG
    }
    elseif ($Item -is [System.Windows.RoutedEventArgs]) {
        "Event: '{0}', Event Type: '{1}" -f $Item.RoutedEvent.Name, $Item.RoutedEvent.OwnerType.FullName | Write-LogOutput -LogType DEBUG
    }
    elseif ($Item -is [System.EventArgs]) {
        "Event: '{0}'" -f $Item.RoutedEvent.Name | Write-LogOutput -LogType DEBUG
    }
    else {
        "Control: {0}, Event: '{1}', Event Type: '{2}'" -f $Item.Source.Name, $Item.RoutedEvent.Name, $Item.RoutedEvent.OwnerType.FullName | Write-LogOutput -LogType DEBUG
    }
}

#endregion

#region controls
[Windows.Forms.Application]::EnableVisualStyles()

$SplashForm = New-Object System.Windows.Forms.Form
$SplashForm.Text = "Loading..."
$SplashForm.FormBorderStyle = [System.Windows.Forms.FormBorderStyle]::None
$SplashForm.StartPosition = [System.Windows.Forms.FormStartPosition]::CenterScreen
$SplashForm.Width = 300
$SplashForm.Height = 250
$SplashForm.BackColor = [System.Drawing.Color]::White

# Create a PictureBox for the logo
$LogoPictureBox = New-Object System.Windows.Forms.PictureBox
$LogoPictureBox.Image = Get-Icon -Type WinForms
$LogoPictureBox.SizeMode = [System.Windows.Forms.PictureBoxSizeMode]::Zoom
$LogoPictureBox.Width = 150
$LogoPictureBox.Height = 150
#$LogoPictureBox.Location = New-Object System.Drawing.Point(($SplashForm.Width - $LogoPictureBox.Width) / 2, 20)
$LogoPictureBox.Location = New-Object System.Drawing.Point(65, 20)
$SplashForm.Controls.Add($LogoPictureBox)

$SplashLabel = New-Object System.Windows.Forms.Label
$SplashLabel.Text = "Initializing application..."
$SplashLabel.Font = New-Object System.Drawing.Font("Segoe UI", 12, [System.Drawing.FontStyle]::Bold)
$SplashLabel.AutoSize = $True
$SplashLabel.Location = New-Object System.Drawing.Point(55, 180)
$SplashForm.Controls.Add($SplashLabel)
#endregion

# Load XAML
[xml]$Xaml = Get-Content (Join-Path $ScriptRootFolder -ChildPath "OmadaSqlTroubleshooter.xaml")
$NamespaceManager = New-Object System.Xml.XmlNamespaceManager($Xaml.NameTable)
$NamespaceManager.AddNamespace("default", "http://schemas.microsoft.com/winfx/2006/xaml/presentation")
$NamespaceManager.AddNamespace("x", "http://schemas.microsoft.com/winfx/2006/xaml")
$NamespaceManager.AddNamespace("Wpf", "clr-namespace:Microsoft.Web.WebView2.Wpf;assembly=Microsoft.Web.WebView2.Wpf")

Add-Type -AssemblyName PresentationFramework
$Reader = (New-Object System.Xml.XmlNodeReader $Xaml)
$Form = [Windows.Markup.XamlReader]::Load($Reader)
$Form.Icon = Get-Icon -Type Wpf

# Access controls
$ElementNames = @("ComboBox", "Label", "TextBox", "Button", "CheckBox", "RadioButton", "PasswordBox", "ComboBoxItem", "WebView2")
foreach ($ElementName in $ElementNames) {
    $Xaml.DocumentElement.SelectNodes("//default:$ElementName", $NamespaceManager) | ForEach-Object {
        $_.Name | Select-Object -Unique | ForEach-Object {
            if (![string]::IsNullOrWhiteSpace($_)) {
                New-Variable -Name $_ -Value $Form.FindName($_) -Force
            }
        }
    }
}
$WebView = $Form.FindName("webView21")

#region events
try {

    #How to lookup events for a button: ([System.Windows.Controls.Button].GetEvents()|where name -eq 'Click').AddMethod.Name

    $Form.Add_Loaded({
            $_ | Show-EventInfo
            try {

                if ($Null -eq $WebView) {
                    [System.Windows.MessageBox]::Show("Failed to find WebView2 control.", "Error", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
                    return
                }

                $Script:WebviewUserDataFolder = Join-Path $Env:TEMP -ChildPath "OmadaSqlTroubleshooter"
                if (-not (Test-Path -Path $Script:WebviewUserDataFolder)) {
                    New-Item -Path $Script:WebviewUserDataFolder -ItemType Directory | Out-Null
                }

                $Environment = [Microsoft.Web.WebView2.Core.CoreWebView2Environment]::CreateAsync($Null, $Script:WebviewUserDataFolder).GetAwaiter().GetResult()

                $WebView.EnsureCoreWebView2Async($Environment).GetAwaiter().OnCompleted({
                        if ($Null -eq $WebView.CoreWebView2) {
                            $Form.Dispatcher.Invoke([System.Action] {
                                    [System.Windows.MessageBox]::Show("WebView2 environment initialization failed.", "Error", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
                                })
                            return
                        }
                        $HtmlFile = Join-Path  $ScriptRootFolder -ChildPath "Monaco\index.html"
                        if ([System.IO.File]::Exists($HtmlFile)) {
                            $WebView.Dispatcher.Invoke([System.Action] {
                                    $WebView.Source = New-Object System.Uri($HtmlFile)
                                    "Webiew source set to: {0}" -f $HtmlFile | Write-LogOutput -LogType DEBUG
                                })
                        }
                        else {
                            $Form.Dispatcher.Invoke([System.Action] {
                                    [System.Windows.MessageBox]::Show("Monaco HTML file not found at: $HtmlPath", "Error", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
                                })
                        }

                    })

            }
            catch {
                [System.Windows.MessageBox]::Show("WebView2 initialization failed: $($_.Exception.Message)", "Error", [System.Windows.MessageBoxButton]::OK, [System.Windows.MessageBoxImage]::Error)
            }
        })

    $WebView.add_NavigationCompleted({
            $_ | Show-EventInfo
            "Set-EditorValue after loading html" | Write-LogOutput -LogType DEBUG
            Set-EditorValue

            #Not working, needs to be investigated
            #Set-EditorBackground

        })


    $CheckboxMyQueriesOnly.Add_Checked({
            $_ | Show-EventInfo
            $True | Invoke-ProcessConfigSettings -Property "MyQueriesOnly"

            $QueryUrl = "{0}/actusersettingsdlg.aspx?HIDEBACKARRICON=1" -f $Script:SqlTroubleShooterConfig.BaseUrl
            $Body = $null
            $Method = "GET"
            $Result = Invoke-OmadaPSWebRequestWrapper

            if ($Result -match [regex]("identityUserName:.\S+")) {
                $Match = $Matches[0]
                $IdentityUserName = $Match.Split(":")[1].Trim().TrimStart("'").TrimEnd(",").TrimEnd("'")
                if (![string]::IsNullOrWhiteSpace($IdentityUserName)) {
                    $IdentityUserName | Invoke-ProcessConfigSettings -Property "IdentityUserName"
                }
            }
            else {
                $null | Invoke-ProcessConfigSettings -Property "IdentityUserName"
            }
            Update-QueryList
        })

    $CheckboxMyQueriesOnly.Add_Unchecked({
            $_ | Show-EventInfo
            $False | Invoke-ProcessConfigSettings -Property "MyQueriesOnly"
            $null | Invoke-ProcessConfigSettings -Property "IdentityUserName"
            Update-QueryList
        })

    $TextBoxURL.Add_LostFocus({
            $_ | Show-EventInfo
            try {
                Set-OmadaUrl
            }
            catch {
                if ($_.Exception.Response.StatusCode -eq "NotFound") {
                    $Message = "SQL Troubleshooting Object not found or OData endpoint for SQL Troubleshooting is not found. Is it enable for OData? Please check the data object type properties!"
                }
                else {
                    $Message = $_.Exception.Message
                }
                $ComboBoxSelectQuery.SelectedItem = $Null
                $ButtonRefreshQueries.IsEnabled = $False
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })

    $ComboBoxSelectAuthenticationOption.Add_GotFocus({
            $_ | Show-EventInfo
            if ($null -eq $ComboBoxSelectAuthenticationOption.SelectedItem) {
                $Script:AuthenticationNotSet = $true
            }
        })

    $ComboBoxSelectAuthenticationOption.Add_SelectionChanged({
            $_ | Show-EventInfo

            Set-AuthenticationOption
            "Changed authentication option to: {0}" -f $Script:SqlTroubleShooterConfig.LastAuthentication | Write-LogOutput -LogType DEBUG

            if ($Script:AuthenticationNotSet -eq $true -and ![string]::IsNullOrWhiteSpace($TextBoxURL.Text)) {
                Set-OmadaUrl
            }

        })

    $ComboBoxSelectQuery.Add_DropDownOpened({
            $_ | Show-EventInfo
            try {
                if (($ComboBoxSelectQuery.Items | Measure-Object).Count -le 1) {
                    Update-QueryList
                    Update-DataConnectionList
                }
            }
            catch {
                if ($_.Exception.Response.StatusCode -eq "NotFound") {
                    $Message = "SQL Troubleshooting Object not found or OData endpoint for SQL Troubleshooting is not found. Is it enable for OData? Please check the data object type properties!"
                }
                else {
                    $Message = $_.Exception.Message
                }
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }

        })

    $ComboBoxSelectQuery.Add_SelectionChanged({
            $_ | Show-EventInfo
            try {
                $Script:CurrentSqlQueryDisplayName = $ComboBoxSelectQuery.SelectedItem.Content
                if (($ComboBoxSelectQuery.Items | Measure-Object).Count -ge 0 -and ![string]::IsNullOrWhiteSpace($ComboBoxSelectQuery.SelectedItem.Content)) {
                    #Update-QueryList
                    #Update-DataConnectionList
                    Set-EditorValue
                    #Not working, needs to be investigated
                    #Set-EditorBackground
                }

            }
            catch {
                if ($_.Exception.Response.StatusCode -eq "NotFound") {
                    $Message = "SQL Troubleshooting Object not found for OData endpoint for SQL Troubleshooting is not found. Is it enable for OData? Please check the data object type properties!"
                }
                else {
                    $Message = $_.Exception.Message
                }
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }

        })

    $ComboBoxSelectDataConnection.Add_SelectionChanged({
            $_ | Show-EventInfo

            try {
                if (($ComboBoxSelectDataConnection.Items | Measure-Object).Count -ge 0 -and ![string]::IsNullOrWhiteSpace($ComboBoxSelectDataConnection.SelectedItem)) {
                    Update-DataConnectionList
                    $ComboBoxSelectDataConnection.SelectedItem | Invoke-ProcessConfigSettings -Property "CurrentDataConnection"
                }
                $ComboBoxSelectDataConnection.SelectedItem.Split(" - ")[0].Trim() | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionId"
                $ComboBoxSelectDataConnection.SelectedItem.Split(" - ")[1].Trim() | Invoke-ProcessConfigSettings -Property "CurrentDataConnectionName"
            }
            catch {
                $Message = $_.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })

    $ComboBoxSelectDataConnection.Add_DropDownOpened({
            $_ | Show-EventInfo
            try {
                if (($ComboBoxSelectDataConnection.Items | Measure-Object).Count -le 0) {
                    Update-DataConnectionList
                }
            }
            catch {
                $Message = $_.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })

    $TextBoxUserName.Add_LostFocus({
            $_ | Show-EventInfo
            if (![string]::IsNullOrWhiteSpace($TextBoxUserName.Text)) {

                $TextBoxUserName.Text | Invoke-ProcessConfigSettings -Property "UserName"
                "Username set to: {0}" -f $Script:SqlTroubleShooterConfig.UserName | Write-LogOutput -LogType DEBUG
                if ($InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    if (![string]::IsNullOrWhiteSpace($TextBoxUserName.Text) -and ![string]::IsNullOrWhiteSpace($TextBoxPassword.Password)) {
                        "Create/Update credential with username {0}" -f $TextBoxUserName.Text | Write-LogOutput -LogType DEBUG
                        $InvokeOmadaRestMethodParam.Credential = [System.Management.Automation.PSCredential]::new($Script:SqlTroubleShooterConfig.UserName, ($TextBoxPassword.Password | ConvertTo-SecureString -AsPlainText -Force))
                    }
                }
                "Username set!" | Write-LogOutput
            }
            if ([string]::IsNullOrWhiteSpace($TextBoxUserName.Text)) {
                if ($InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    "Clear credential because username is empty" | Write-LogOutput -LogType DEBUG
                    $InvokeOmadaRestMethodParam.Credential = $Null
                }
                "Username cannot be empty!" | Write-LogOutput -LogType DEBUG
            }
        })

    $TextBoxPassword.Add_LostFocus({
            $_ | Show-EventInfo
            if (![string]::IsNullOrWhiteSpace($TextBoxUserName.Text)) {

                "Password set to: {0}" -f "***********" | Write-LogOutput -LogType DEBUG
                if ($InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    if (![string]::IsNullOrWhiteSpace($TextBoxUserName.Text) -and ![string]::IsNullOrWhiteSpace($TextBoxPassword.Password)) {
                        "Create/Update credential with username {0}" -f $TextBoxUserName.Text | Write-LogOutput -LogType DEBUG
                        $TextBoxUserName.Text | Invoke-ProcessConfigSettings -Property "UserName"
                        $InvokeOmadaRestMethodParam.Credential = [System.Management.Automation.PSCredential]::new($Script:SqlTroubleShooterConfig.UserName, ($TextBoxPassword.Password | ConvertTo-SecureString -AsPlainText -Force))
                    }
                }
                "Password set" | Write-LogOutput
            }
            if ([string]::IsNullOrWhiteSpace($TextBoxPassword.Password)) {
                if ($InvokeOmadaRestMethodParam.ContainsKey("Credential")) {
                    "Clear credential because password is empty" | Write-LogOutput -LogType DEBUG
                    $InvokeOmadaRestMethodParam.Credential = $Null
                }
                "Password cannot be empty!" | Write-LogOutput
            }
        })

    $ButtonRefreshQueries.Add_Click({
            $_ | Show-EventInfo

            try {
                Update-QueryList
            }
            catch {
                if ($_.Exception.Response.StatusCode -eq "NotFound") {
                    $Message = "SQL Troubleshooting Object not found or OData endpoint for SQL Troubleshooting is not found. Is it enable for OData? Please check the data object type properties!"
                }
                else {
                    $Message = $_.Exception.Message
                }
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })

    $ButtonSaveQuery.Add_Click({
            $_ | Show-EventInfo
            $ButtonSaveQuery.IsEnabled = $False
            $ButtonExecuteQuery.IsEnabled = $False
            try {
                if ([string]::IsNullOrWhiteSpace($TextBoxURL.Text) -or [string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.SqlQueryDoId)) {
                    $Message = "Omada Url not set or Query not selected, cannot retrieve data!"
                    $Message | Write-LogOutput -LogType WARNING
                    [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                }
                else {

                    if ($InvokeOmadaRestMethodParam.ContainsKey("Credential") -and $Null -eq $InvokeOmadaRestMethodParam.Credential) {
                        $Message = "Credential is not present, please check credential input!"
                        $Message | Write-LogOutput -LogType ERROR
                        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    }

                    "Save query" | Write-LogOutput

                    Invoke-SaveQuery
                }
            }
            catch {
                $Message = $_.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })


    $ButtonExecuteQuery.Add_Click({
            $_ | Show-EventInfo
            $ButtonSaveQuery.IsEnabled = $False
            $ButtonExecuteQuery.IsEnabled = $False
            $ButtonShowOutput.IsEnabled = $False
            $ButtonSaveOutputFile.IsEnabled = $False
            try {
                if ([string]::IsNullOrWhiteSpace($TextBoxURL.Text) -or [string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.SqlQueryDoId)) {
                    $Message = "Omada Url not set or Query not selected, cannot retrieve data!"
                    $Message | Write-LogOutput -LogType WARNING
                    [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                }
                else {

                    if ($InvokeOmadaRestMethodParam.ContainsKey("Credential") -and $Null -eq $InvokeOmadaRestMethodParam.Credential) {
                        $Message = "Credential is not present, please check credential input!"
                        $Message | Write-LogOutput -LogType ERROR
                        [System.Windows.Forms.MessageBox]::Show(("Failure occurred:`r`n`r`n{0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
                    }

                    "Execute query" | Write-LogOutput

                    Invoke-SaveAndExecuteQuery
                }
            }
            catch {
                $Message = $_.Exception.Message
                $Message | Write-LogOutput -LogType ERROR
                [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
            }
        })


    $ButtonSaveOutputFile.Add_Click({
            $_ | Show-EventInfo

            $SaveFileDialog = New-Object System.Windows.Forms.SaveFileDialog
            $SaveFileDialog.Filter = "Json files (*.json) | *.json | Csv files (*.csv) | *.csv | CliXml files (*.xml) | *.xml | Text files (*.txt) | *.txt | All files (*.*) | *.*"
            $SaveFileDialog.Title = "Save Output File"
            if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.LastOutputFolder)) {
                $SaveFileDialog.InitialDirectory = $Script:SqlTroubleShooterConfig.LastOutputFolder
            }
            if ([string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.LastOutputFolder)) {
                ".json" | Invoke-ProcessConfigSettings -Property "LastExtension"
            }
            $SaveFileDialog.DefaultExt = $Script:SqlTroubleShooterConfig.LastExtension
            $InvalidFileNameChars = [System.IO.Path]::GetInvalidFileNameChars()
            $SaveFileDisplayName = $TextBoxDisplayName.Text
            if (![string]::IsNullOrWhiteSpace($SaveFileDisplayName)) {
                $SaveFileDisplayName = ($SaveFileDisplayName.ToCharArray() | ForEach-Object {
                        if ($InvalidFileNameChars -contains $_) {
                            "_"
                        }
                        $_
                    }) -Join ""
            }
            else {
                $SaveFileDisplayName = "Output"
            }
            $SaveFileDialog.FileName = "SqlQuery_{0}_{1}_{2}_{3}_Output{4}" -f $Script:SqlTroubleShooterConfig.SqlQueryDoId, $SaveFileDisplayName, $Script:SqlTroubleShooterConfig.CurrentDataConnectionName, [system.uri]::New($Script:SqlTroubleShooterConfig.BaseUrl).Host, $Script:SqlTroubleShooterConfig.LastExtension
            if ($SaveFileDialog.ShowDialog() -eq [System.Windows.Forms.DialogResult]::OK) {
                $Script:OutputFileName = $SaveFileDialog.FileName
                "Save outputfile: {0}" -f $Script:OutputFileName | Write-LogOutput

                if ($Null -eq $Script:OutputFileName) {
                    return
                }
                elseif ($Script:OutputFileName -like "*.json") {
                    $Script:QueryResult | ConvertTo-Json -Depth 15 | Set-Content $Script:OutputFileName -Encoding UTF8
                }
                elseif ($Script:OutputFileName -like "*.csv") {
                    $Script:QueryResult.d.rows | Export-Csv -Path $Script:OutputFileName -Delimiter ";" -NoTypeInformation -Encoding UTF8
                }
                elseif ($Script:OutputFileName -like "* .xml") {
                    $Script:QueryResult | Export-Clixml -Path $Script:OutputFileName -Depth 15
                }
                else {
                ($Script:QueryResult.d.rows | Format-Table -AutoSize | Out-String -Width 10000000).Trim() | Set-Content $Script:OutputFileName -Encoding UTF8
                }

                "Output file saved!" | Write-LogOutput -LogType DEBUG
                Split-Path $Script:OutputFileName | Invoke-ProcessConfigSettings -Property "LastOutputFolder"
                [System.IO.Path]::GetExtension($Script:OutputFileName) | Invoke-ProcessConfigSettings -Property "LastExtension"
                $ButtonOpenOutputFile.IsEnabled = $True
            }
            else {
                "File was not saved!" | Write-LogOutput -LogType DEBUG
            }
        })

    $ButtonShowOutput.Add_Click({
            $_ | Show-EventInfo
            "Show output" | Write-LogOutput
            $Script:QueryResult.d.rows | Out-GridView -Title ("{0} - {1}" -f $Form.Text, $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId)
        })

    $ButtonOpenOutputFile.Add_Click({
            $_ | Show-EventInfo
            "Open outputfile: {0}" -f $Script:OutputFileName | Write-LogOutput
            & $Script:OutputFileName
        })

    $Script:OutputFileName

    $ButtonReset.Add_Click({
            $_ | Show-EventInfo
            $ButtonExecuteQuery.IsEnabled = $False
            $ButtonSaveOutputFile.IsEnabled = $False
            $ButtonOpenOutputFile.IsEnabled = $False
            $ButtonReset.IsEnabled = $False
            $TextBoxURL.IsEnabled = $True
            $TextBoxDisplayName.IsEnabled = $False
            $TextBoxDisplayName.Text = ""
            $ButtonShowOutput.IsEnabled = $False
            $ComboBoxSelectQuery.SelectedItem = $Null
            $ButtonSaveQuery.IsEnabled = $False
            "Reset" | Write-LogOutput
        })

    $ButtonClearLog.Add_Click({
            $_ | Show-EventInfo
            $TextBoxOutput.Text = ""
            "Log cleared" | Write-LogOutput
        })

    $ComboBoxSelectLogLevel.Add_SelectionChanged({
            $_ | Show-EventInfo
            $ComboBoxSelectLogLevel.SelectedItem.Content | Invoke-ProcessConfigSettings -Property "LogLevel"
            $Script:LogLevelSetting = $ComboBoxSelectLogLevel.SelectedItem.Content
            "Logging set to {0}!" -f $Script:LogLevelSetting | Write-LogOutput -LogType LOG
        })
}
catch {
    if ($_.Exception.Response.StatusCode -eq "NotFound") {
        $Message = "SQL Troubleshooting Object not found or OData endpoint for SQL Troubleshooting is not found. Is it enable for OData? Please check the data object type properties!"
    }
    else {
        $Message = $_.Exception.Message
    }
    $Message | Write-LogOutput -LogType ERROR
    [System.Windows.Forms.MessageBox]::Show(("Failure occured:`r`n {0}" -f $Message), "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}
#endregion

#region process
try {
    [void]$SplashForm.Show()
    $ApplicationTitle = $Form.Title.ToString()
    "Application '{0}': Start initialization..." -f $ApplicationTitle | Write-Host -ForegroundColor Green
    Invoke-ProcessConfigSettings
    [System.Windows.Forms.Application]::DoEvents()
    $Result = $Null

    $Script:OutputFileName = $Null
    $SqlQueryObject = $Null
    $Script:CurrentSqlQueryDisplayName = $Null
    $CurrentUrl = $Null

    $TextBoxURL.Text = $Script:SqlTroubleShooterConfig.BaseUrl
    $TextBoxURL.IsEnabled = $True
    if (![String]::IsNullOrWhiteSpace($TextBoxURL.Text)) {
        $CurrentUrl = $TextBoxURL.Text
    }

    $InvokeOmadaRestMethodParam = @{
        Uri                = $Null
        Method             = "GET"
        AuthenticationType = $($Script:SqlTroubleShooterConfig.LastAuthentication)
    }
    $Password = $Null

    if ($Script:SqlTroubleShooterConfig.MyQueriesOnly) {
        $CheckboxMyQueriesOnly.IsChecked = $True
    }

    if ($null -ne $Script:LogLevelSetting) {
        $Script:LogLevelSetting | Invoke-ProcessConfigSettings -Property "LogLevel"
    }
    if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.LogLevel)) {
        if (($ComboBoxSelectLogLevel.Items | Measure-Object).count -le 0 -and !$ComboBoxSelectLogLevel.Items.Content.Contains($Script:SqlTroubleShooterConfig.LogLevel)) {
            $ComboBoxSelectLogLevelItem = New-Object System.Windows.Controls.ComboBoxItem
            $ComboBoxSelectLogLevelItem.Content = $Script:SqlTroubleShooterConfig.LogLevel
            $ComboBoxSelectLogLevel.Items.Add($ComboBoxSelectLogLevelItem) | Out-Null
        }
        $ComboBoxSelectLogLevel.SelectedValue = $ComboBoxSelectLogLevel.Items | Where-Object { $_.Content -eq $Script:SqlTroubleShooterConfig.LogLevel }
        $Script:LogLevelSetting = $Script:SqlTroubleShooterConfig.LogLevel
    }
    else {
        if (($ComboBoxSelectLogLevel.Items | Measure-Object).count -le 0 -and !$ComboBoxSelectLogLevel.Items.Content.Contains("INFO")) {
            $ComboBoxSelectLogLevelItem = New-Object System.Windows.Controls.ComboBoxItem
            $ComboBoxSelectLogLevelItem.Content = "INFO"
            $ComboBoxSelectLogLevel.Items.Add($ComboBoxSelectLogLevelItem) | Out-Null
        }
        $ComboBoxSelectLogLevel.SelectedValue = $ComboBoxSelectLogLevel.Items | Where-Object { $_.Content -eq "INFO" }
        $Script:LogLevelSetting = $ComboBoxSelectLogLevel.SelectedValue.Content
    }

    if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId)) {
        $ComboBoxSelectQueryItem = $null
        $ComboBoxSelectQueryItem = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId }
        if ($null -eq $ComboBoxSelectQueryItem) {
            $ComboBoxSelectQueryItem = New-Object System.Windows.Controls.ComboBoxItem
            $ComboBoxSelectQueryItem.Content = $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId
            $ComboBoxSelectQuery.Items.Add($ComboBoxSelectQueryItem) | Out-Null
            $Script:CurrentSqlQueryDisplayName = $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId.Split(" - ")[1].Trim()
            $TextBoxDisplayName.Text = $Script:CurrentSqlQueryDisplayName
        }
        $ComboBoxSelectQuery.SelectedValue = $ComboBoxSelectQuery.Items | Where-Object { $_.Content -eq $Script:SqlTroubleShooterConfig.SelectedSqlQueryDoId }
    }

    if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.CurrentDataConnection)) {
        Set-DataConnection
    }

    if ([string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.LastAuthentication)) {
        $ComboBoxSelectAuthenticationOption.SelectedValue = $Script:SqlTroubleShooterConfig.LastAuthentication
    }

    if (![string]::IsNullOrWhiteSpace($Script:SqlTroubleShooterConfig.UserName)) {
        $TextBoxUserName.Text = $Script:SqlTroubleShooterConfig.UserName
    }
    Set-OmadaUrl
    Set-AuthenticationOption
    $SplashForm.Hide()
    $SplashForm.Dispose()
}
catch {
    Remove-Variable Webview
    $Message = $_.Exception.Message
    $Message | Write-LogOutput -LogType ERROR

    [System.Windows.Forms.MessageBox]::Show("An error occurred: " + $Message, "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}

try {
    "Application '{0}': Initialized!" -f $ApplicationTitle | Write-Host -ForegroundColor Green
    [void]$Form.ShowDialog()
    "Application '{0}': Closed, cleaning-up!" -f $ApplicationTitle | Write-Host -ForegroundColor Green
    Invoke-ProcessConfigSettings
    $Form.Close() | Out-Null
    $WebView.Dispose() | Out-Null
}
catch {

    $Message = $_.Exception.Message
    $Message | Write-LogOutput -LogType ERROR
    [System.Windows.Forms.MessageBox]::Show("An error occurred: " + $Message, "Error", [System.Windows.Forms.MessageBoxButtons]::OK, [System.Windows.Forms.MessageBoxIcon]::Error)
}

Pop-Location
$EndVariables = Get-Variable
$SkipVariableNames = @("WshShell","WhatIfPreference","WarningPreference","VerbosePreference","true","PSItem","Task")
foreach ($EndVariable in $EndVariables) {
    if ($EndVariable.Name -notin $StartVariables.Name -and $EndVariable.Name -notin $SkipVariableNames) {
        try {
            Remove-Variable -Name $EndVariable.Name -Force -ErrorAction SilentlyContinue
        }
        catch {}
    }
}
"Application '{0}': Clean-up complete!" -f $ApplicationTitle | Write-Host -ForegroundColor Green
#endregion
